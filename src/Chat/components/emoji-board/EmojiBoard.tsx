...
function RecentEmojiSidebarStack({
  onItemClick
}: {
    ...
    <SidebarStack>
      ...
      >
        <Icon
          ...
          filled={activeGroupId === RECENT_GROUP_ID}
        />
      </SidebarBtn>
    </SidebarStack>
  )
        ...
        return (
          ...
          >
            <img
              ...
              src={
                mx.mxcUrlToHttp(pack.getPackAvatarUrl(usage) ?? "") ||
                pack.avatarUrl
              }
    ...
    <>
      {groups.map((pack) => (
        <EmojiGroup
          key={pack.id}
          id={pack.id}
          label={pack.displayName || "Unknown"}
        >
...
export const StickerGroups = memo(({ mx, groups, fetchGifs, searchQuery }) => {
    ...
    <>
      ...
      <Grid
        ...
        onGifClick={(gif) => {
          ...
          //     },
          //     url: {
          //       content_uri: gif?.images?.downsized?.url
          //     }
          //   }
          // ])
          // setDraft((prevState) => {
...
export function EmojiBoard({
  ...
  onStickerSelect?: (url: string, shortcode: string, label: string) => void
  ...
  const syncActiveGroupId = useCallback(() => {
    ...
    if (!targetEl) return
    const groupEls = [
      ...targetEl.querySelectorAll("div[data-group-id]")
    ] as HTMLElement[]
    const groupEl = groupEls.find((el) =>
      isIntersectingScrollView(targetEl, el)
    )
    const groupId = groupEl?.getAttribute("data-group-id") ?? undefined
    setActiveGroupId(groupId)
  }, [setActiveGroupId])

  const handleOnScroll: UIEventHandler<HTMLDivElement> = useThrottle(
    syncActiveGroupId,
    {
      wait: 500
    }
  ...
  const handleEmojiPreview = useCallback(
    (element: HTMLButtonElement) => {
      ...
      if (emojiInfo.type === EmojiType.Emoji && emojiPreviewRef.current) {
        ...
        emojiInfo.type === EmojiType.CustomEmoji &&
        emojiPreviewRef.current
      ) {
        const img = document.createElement("img")
        img.className = css.CustomEmojiImg
        img.setAttribute(
          "src",
          mx.mxcUrlToHttp(emojiInfo.data) || emojiInfo.data
        )
          ...
          <Header>
            <Box direction="Column" gap="200">
              ...
              <Input
                ...
                size="400"
                placeholder={
                  allowTextCustomEmoji ? "Search or Text Reaction " : "Search"
                }
                maxLength={50}
                after={
            ...
            >
              {result && emojiTab && (
                <SearchEmojiGroup
                  ...
                  id={SEARCH_GROUP_ID}
                  label={
                    result.items.length ? "Search Results" : "No Results found"
                  }
                  emojis={result.items}
                />
              )}
