...
const roomAliasFromQueryText = (mx: MatrixClient, text: string) =>
  validMxId(`#${text}`)
    ? `#${text}`
    : `#${text}${text.endsWith(":") ? "" : ":"}${getMxIdServer(
        mx.getUserId() ?? ""
      )}`
...
export function RoomMentionAutocomplete({
  ...
  const handleAutocomplete: MentionAutoCompleteHandler = (
    roomAliasOrId,
    name
  ) => {
    const mentionEl = createMentionElement(
      ...
      roomId === roomAliasOrId ||
        mx.getRoom(roomId)?.getCanonicalAlias() === roomAliasOrId
    )
    ...
    >
      {autoCompleteRoomIds.length === 0 ? (
        ...
        autoCompleteRoomIds.map((rId) => {
          ...
          const iconSrc =
            !dm &&
            joinRuleToIconSrc(Icons, room.getJoinRule(), room.isSpaceRoom())

          const handleSelect = () =>
            handleAutocomplete(room.getCanonicalAlias() ?? rId, room.name)
            ...
            <MenuItem
              ...
              before={
                <Avatar size="200">
                  ...
                  {avatarUrl && !iconSrc && (
                    <AvatarImage src={avatarUrl} alt={room.name} />
                  )}
                  {!avatarUrl && !iconSrc && (
                    <AvatarFallback
                      style={{
