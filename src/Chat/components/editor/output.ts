...
const elementToCustomHtml = (node: CustomElement, children: string): string => {
  switch (node.type) {
    ...
    case BlockType.Mention:
      ...
      //   node.name
      // )}</a>`;
      const username = node.name.slice(1)
      return `<span type="mentions" id="${node?.id}">${username} </span>`
...
export const toMatrixCustomHTML = (
  ...
  const parseNode = (
    ...
    return `${parsedMarkdown}${toMatrixCustomHTML(n, {
      ...opts,
      allowBlockMarkdown: false
    })}`
  }
...
export const customHtmlEqualsPlainText = (
  customHtml: string,
  plain: string
): boolean => customHtml.replace(/<br\/>/g, "\n") === sanitizeText(plain)

export const trimCustomHtml = (customHtml: string) =>
  customHtml.replace(/<br\/>$/g, "").trim()
