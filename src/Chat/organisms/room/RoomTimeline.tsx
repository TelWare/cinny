...
export const getLinkedTimelines = (
  timeline: EventTimeline
): EventTimeline[] => {
...
export const getTimelineEvent = (
  timeline: EventTimeline,
  index: number
): MatrixEvent | undefined => timeline.getEvents()[index]
...
export const getEventIdAbsoluteIndex = (
  ...
  const eventIndex = eventTimeline
    .getEvents()
    .findIndex((evt) => evt.getId() === eventId)
...
export const factoryGetFileSrcUrl =
  (httpUrl: string, mimeType: string, encFile?: IEncryptedFile) =>
...
const useEventTimelineLoader = (
  ...
  const loadEventTimeline = useCallback(
    async (eventId: string) => {
      ...
      const absIndex = getEventIdAbsoluteIndex(
        linkedTimelines,
        replyEvtTimeline,
        eventId
      )
...
const useTimelinePagination = (
  ...
  const handleTimelinePagination = useMemo(() => {
    ...
    const recalibratePagination = (
      ...
      const timelineMatch = (mt: EventTimeline) => (t: EventTimeline) =>
        t === mt

      const newLTimelines = getLinkedTimelines(topTimeline)
      const topTmIndex = newLTimelines.findIndex(timelineMatch(topTimeline))
      const topAddedTm =
        topTmIndex === -1 ? [] : newLTimelines.slice(0, topTmIndex)

      const topTmAddedEvt =
        timelineToEventsCount(newLTimelines[topTmIndex]) -
        timelinesEventsCount[0]
      const offsetRange =
        getTimelinesEventsCount(topAddedTm) + (backwards ? topTmAddedEvt : 0)
    ...
    return async (backwards: boolean) => {
      ...
      const timelineToPaginate = backwards
        ? lTimelines[0]
        : lTimelines[lTimelines.length - 1]
...
const useLiveTimelineRefresh = (room: Room, onRefresh: () => void) => {
  useEffect(() => {
    const handleTimelineRefresh: RoomEventHandlerMap[RoomEvent.TimelineRefresh] =
      (r) => {
...
export function RoomTimeline({
  ...
  const canPaginateBack =
    typeof timeline.linkedTimelines[0]?.getPaginationToken(
      Direction.Backward
    ) === "string"
  ...
  useResizeObserver(
    useMemo(() => {
      ...
      return (entries) => {
        ...
        const editorBaseEntry = getResizeObserverEntry(
    ...
    useCallback(
      (evt) => {
        if (
          ...
          document.activeElement?.getAttribute("data-editable-name") ===
            "RoomInput" &&
          isEmptyEditor(editor)
        ) {
          const editableEvt = getLatestEditableEvt(
            room.getLiveTimeline(),
            (mEvt) => canEditEvent(mx, mEvt)
          )
  ...
  useLayoutEffect(() => {
    ...
    if (readUptoEventId && inLiveTimeline && scrollTo) {
      ...
      const absoluteIndex =
        evtTimeline &&
        getEventIdAbsoluteIndex(linkedTimelines, evtTimeline, readUptoEventId)
      if (absoluteIndex) {
  ...
  useLayoutEffect(() => {
    if (scrollToBottomCount > 0) {
      ...
      if (scrollEl)
        scrollToBottom(
          scrollEl,
          scrollToBottomRef.current.smooth ? "smooth" : "instant"
        )
    }
  }, [scrollToBottomCount])
  ...
  useEffect(() => {
    ...
    initMatrix.notifications?.on(
      cons.events.notifications.FULL_READ,
      handleFullRead
    )
    return () => {
      initMatrix.notifications?.removeListener(
        cons.events.notifications.FULL_READ,
        handleFullRead
      )
    }
  ...
  const handleReplyClick: MouseEventHandler<HTMLButtonElement> = useCallback(
    (evt) => {
      ...
      if (!replyEvt) return
      const editedReply = getEditedEvent(
        replyId,
        replyEvt,
  ...
  const handleReactionToggle = useCallback(
    (targetEventId: string, key: string, shortcode?: string) => {
      ...
      const rShortcode =
        shortcode ||
        (reactions.find(eventWithShortcode)?.getContent().shortcode as
          | string
          | undefined)
      mx.sendEvent(
        room.roomId,
  ...
  const renderRoomMsgContent = useRoomMsgContentRenderer<[EventTimelineSet]>({
    renderText: (mEventId, mEvent, timelineSet) => {
      ...
      return (
        <>
          ...
          {msgType == MsgType.File && fileRenderer(mEventId, mEvent)}
        </>
      )
    },
    ...
    renderUnsupported: (mEventId, mEvent) => {
      if (mEvent.isRedacted()) {
        ...
        const reason =
          redactedEvt && "content" in redactedEvt
            ? redactedEvt.content.reason
            : undefined
  ...
  const renderMatrixEvent = useMatrixEventRenderer<
    [number, EventTimelineSet, boolean]
  >({
    renderRoomMessage: (mEventId, mEvent, item, timelineSet, collapse) => {
      const reactionRelations = getEventReactions(timelineSet, mEventId)
      const reactions =
        reactionRelations && reactionRelations.getSortedAnnotationsByKey()
      const hasReactions = reactions && reactions.length > 0
      const { replyEventId } = mEvent
      const highlighted =
        focusItem.current?.index === item && focusItem.current.highlight
      console.log(mEvent?.isThreadRoot, "threadroot")
      let threadsTimeline
      ...
      if (mEvent?.isThreadRoot) {
        threadsTimeline = room?.getThread(mEvent?.getId())?.timeline

        console.log(threadsTimeline?.length, "threadroot length")
      }
      return (
        <>
          <Message
          ...
          {room.getType() === "Channels" && (
            <div
              className="roomreplymsg"
              onClick={() => {
                setIsThreadOpen(true)
                setThreadEventId(mEvent.getId())
              }}
            >
              <button>
                {threadsTimeline?.length
                  ? `${threadsTimeline?.length} Replies`
                  : "Reply"}
                {/* {<span> Replies</span>} */}
              </button>
            </div>
  ...
  return (
    <Box
      style={{ height: "100%", color: color.Surface.OnContainer }}
      grow="Yes"
    >
