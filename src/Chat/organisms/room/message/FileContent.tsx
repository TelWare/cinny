...
function DownloadFile({
  ...
  const [downloadState, download] = useAsyncCallback(
    useCallback(async () => {
      ...
      FileSaver.saveAs(httpUrl, newBody)
      return httpUrl
    }, [mx, url, mimeType, encInfo, newBody])
  )

  return downloadState.status === AsyncStatus.Error ? (
    renderErrorButton(
      download,
      `Retry Download (${bytesToSize(info.size ?? 0)})`
    )
  ) : (
    <Button
      variant="Secondary"
      ...
      onClick={() =>
        downloadState.status === AsyncStatus.Success
          ? FileSaver.saveAs(downloadState.data, newBody)
          : download()
      }
    ...
    >
      <Text size="B400" truncate>{`Download (${bytesToSize(
        info.size ?? 0
      )})`}</Text>
    </Button>
  )
