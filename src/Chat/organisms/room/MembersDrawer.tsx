...
export function MembersDrawer({ room }: MembersDrawerProps) {
  ...
  const [sortFilterIndex, setSortFilterIndex] = useSetting(
    settingsAtom,
  ...
  const membershipFilter =
    membershipFilterMenu[membershipFilterIndex] ?? membershipFilterMenu[0]
  ...
  const typingMembers = useAtomValue(
    useMemo(
      () => selectRoomTypingMembersAtom(room.roomId, roomIdToTypingMembersAtom),
      [room]
    )
  )
  ...
  if (!result && searchInputRef.current?.value)
  ...
  useIntersectionObserver(
    useCallback((intersectionEntries) => {
      ...
      const entry = getIntersectionObserverEntry(
        scrollTopAnchorRef.current,
        intersectionEntries
      )
                ...
                <Input
                  ...
                  after={
                    result && (
                      <Chip
                        variant={
                          result.items.length > 0 ? "Success" : "Critical"
                        }
                        size="400"
                        radii="Pill"
                        aria-pressed
