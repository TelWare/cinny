...
export const ReactionViewer = as<"div", ReactionViewerProps>(
  ({ className, room, initialKey, relations, requestClose, ...props }, ref) => {
    ...
    const [selectedKey, setSelectedKey] = useState<string>(() => {
      ...
      const defaultReaction = reactions.find(
        (reaction) => typeof reaction[0] === "string"
      )
          ...
          <Header className={css.Header} variant="Surface" size="600">
            <Box grow="Yes">
              <Text
                size="H3"
                truncate
              >{`Reacted with :${selectedShortcode}:`}</Text>
            </Box>
            ...
            <Scroll visibility="Hover" hideTrack size="300">
              <Box className={css.Content} direction="Column">
                {selectedReactions.map((mEvent) => {
                  ...
                  const name =
                    (member ? getName(member) : getMxIdLocalPart(senderId)) ??
                    senderId
