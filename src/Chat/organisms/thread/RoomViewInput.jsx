...
function RoomViewInput({ roomId, roomTimeline, viewEvent }) {
  ...
  function setUpReply(userId, eventId, body, formattedBody, relatesTo) {
  ...
  const processCommand = async (cmdBody) => {
    ...
    if (!commands[cmdName]) {
      const sendAsMessage = await confirmDialog(
        "Invalid Command",
        `"${cmdName}" is not a valid command. Did you mean to send this as a message?`,
        "Send as message"
      )
      if (sendAsMessage) {
        sendBody(cmdBody)
  ...
  function renderInputs() {
    ...
    const tombstoneEvent =
      roomTimeline.room.currentState.getStateEvents("m.room.tombstone")[0]
