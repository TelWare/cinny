...
export const getLiveTimeline = (room: Room, eventId: string, mx?: MatrixClient): EventTimeline => {
  const threadsTimeline = room.getThread(eventId)?.getUnfilteredTimelineSet().getLiveTimeline();
...
export const getEventTimeline = (room: Room, eventId: string): EventTimeline | undefined => {
  ...
  const timelineSet = room?.getThread(eventId)?.getUnfilteredTimelineSet();
  return timelineSet?.getTimelineForEvent(eventId) ?? undefined;
  // return room?.getThread(eventId)?.timeline ?? undefined
};
  ...
  const loadEventTimeline = useCallback(
    async (eventId: string) => {
      const [err, replyEvtTimeline] = await to(
        mx.getEventTimeline(room?.getThread(eventId)?.getUnfilteredTimelineSet(), eventId)
      );
      if (!replyEvtTimeline) {
        onError(err ?? null);
        return;
...
const getInitialTimeline = (room: Room, eventId: string, mx?: MatrixClient) => {
  const linkedTimelines = getLinkedTimelines(getLiveTimeline(room, eventId, mx));
  // const linkedTimelines = room.getThread(eventId)?.timeline
...
export function RoomTimeline({ room, eventId, roomInputRef, editor }: RoomTimelineProps) {
  ...
  const [timeline, setTimeline] = useState<Timeline>(() =>
    // room.getThread("$QBYkbjZedGF0ES5QkHVKLeVgGh0WxXx-eL3o2JN97ys")?.timeline
    eventId ? getEmptyTimeline() : getInitialTimeline(room, eventId, mx)
  );

  const eventsLength = getTimelinesEventsCount(timeline.linkedTimelines);
  const liveTimelineLinked =
    timeline.linkedTimelines[timeline.linkedTimelines.length - 1] ===
    getLiveTimeline(room, eventId, mx);
  ...
  const handleJumpToLatest = () => {
    setTimeline(getInitialTimeline(room, eventId, mx));
    scrollToBottomRef.current.count += 1;
    scrollToBottomRef.current.smooth = false;
  };
  ...
  const renderRoomMsgContent = useRoomMsgContentRenderer<[EventTimelineSet]>({
    renderText: (mEventId, mEvent, timelineSet) => {
      ...
      if (content.attachments?.length) {
        ...
        msgType = content?.attachments ? content?.attachments[0]?.info.mtype : MsgType.File;
        ...
        <>
          ...
          {msgType == MsgType.Audio && (
            <Attachment>
              ...
              <AttachmentBox>
                <AttachmentContent>
                  <AudioContent info={file} mimeType={safeMimeType} url={mediaMXC} encInfo={file} />
                </AttachmentContent>
              </AttachmentBox>
            </Attachment>
          )}
  ...
  const eventRenderer = (item: number) => {
    ...
    const isChannelReply = mEvent?.getContent()?.['m.relates_to']?.rel_type === 'm.thread';
    if ((mEvent?.getType() === 'm.room.message' && isChannelReply) || mEvent?.isThreadRoot) {
      // const allThreadsTimeline = room?.getThread(mEvent?.getId())?.timeline
    }
