...
export const ThreadInput = forwardRef<HTMLDivElement, RoomInputProps>(
  ({ editor, roomViewRef, roomId, room }, ref) => {
    ...
    const { threadEventId, isThreadOpen, setIsThreadOpen, setThreadEventId } =
      useContext(ChatWrapperContext)
    const threadEventIdRef = useRef()

    const [selectedFiles, setSelectedFiles] = useAtom(
      roomIdToUploadItemsAtomFamily(threadEventId)
    )
    ...
    const imagePackRooms: Room[] = useMemo(() => {
      const allParentSpaces = [
        roomId,
        ...(initMatrix.roomList?.getAllParentSpaces(roomId) ?? [])
      ]
    ...
    useEffect(() => {
      threadEventIdRef.current = threadEventId
    }, [threadEventId, isThreadOpen])
    ...
    const submit = useCallback(async () => {
      ...
      const content: IContent = {
        ...
        body,
        "m.relates_to": {
          event_id: threadEventIdRef.current,
      ...
      if (!customHtmlEqualsPlainText(formattedBody, body)) {
      ...
      const threadsTimeline = room
        .getThread(threadEventIdRef.current)
        ?.getUnfilteredTimelineSet()
        .getLiveTimeline()
      // if (!threadsTimeline?.length) {

      //   if (room && threadEventId) {
      //     const thread = room.createThread(threadEventId, undefined, undefined, true)
      // }
      if (!threadsTimeline?.events?.length) {
        if (room && threadEventIdRef.current) {
          const thread = room.createThread(
            threadEventIdRef.current,
            undefined,
            undefined,
            true
          )
        ...
        .then(() => {
          if (!threadsTimeline?.events?.length) {
            setTimeout(() => {
              ...
              setThreadEventId(threadEventIdRef.current)
            }, 100)
          }
        })
        ...
        <CustomEditor
              ...
              <div>
                ...
                >
                  ...
                  <MessageReply
                    ...
                    name={
                      room?.getMember(replyDraft.userId)?.name ??
                      replyDraft.userId
                    }
          ...
          after={
            <>
              ...
              >
                <Icon
                  src={toolbar ? Icons.AlphabetUnderline : Icons.Alphabet}
                />
              </IconButton>
