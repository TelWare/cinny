...
function Home({ spaceId, size }) {
  ...
  const [roomSortedIds, setRoomSortedIds] = useState([])
  ...
  // useEffect(() => {
  //   setRoomSortedIds([...rooms].sort(roomIdByActivity))
  //   console.log({ size })
  // }, [size])

  // useEffect(() => {
  //   const handleTimeline = (event, room, toStartOfTimeline, removed, data) => {
  //     if (!roomList.rooms.has(room.roomId)) return
  //     if (!data.liveEvent) return
  //     if (roomSortedIds[0] === room.roomId) return
  //     const newDirectIds = [room.roomId]
  //     roomSortedIds.forEach((id) => {
  //       if (id === room.roomId) return
  //       newDirectIds.push(id)
  //     })
  //     setRoomSortedIds(newDirectIds)
  //   }
  ...
  return (
    <>
      ...
      {roomIds.length !== 0 && (
        <>
          {roomList.favouritesChannel.size !== 0 && (
            <Pinned
              size={roomList.favouritesChannel.size}
              roomType={cons.roomType.Channel}
            />
          )}
          <RoomsCategory
            name="Channels"
      ...
      {/* {roomSortedIds.length !== 0 && (
        <>
          ...
          <RoomsCategory
            name="Channels"
            // roomIds={roomIds.sort(roomIdByAtoZ)}
            roomIds={roomSortedIds.sort(roomIdByActivity)}
            drawerPostie={drawerPostie}
          />
        </>
      )} */}
      ...
      {isCategorized &&
        ...
        [...categories.keys()].sort(roomIdByActivity).map((catId) => {
          const rms = []
          const dms = []
