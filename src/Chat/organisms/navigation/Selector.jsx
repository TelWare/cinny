...
function Selector({
  roomId, drawerPostie, onClick,
}) {
  const mx = initMatrix.matrixClient;
  const noti = initMatrix.notifications;
  const room = mx.getRoom(roomId);
  const isDM = room.getType() !== "Channels";

  let imageSrc = room.getAvatarFallbackMember()?.user?.avatarUrl //getAvatarUrl(mx.baseUrl, 24, 24, 'crop') || null;
  if (!imageSrc) imageSrc = room.getAvatarUrl(mx.baseUrl, 24, 24, 'crop') || null;
