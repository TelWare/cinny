  ...
  const opts: HTMLReactParserOptions = {
    replace: (domNode) => {
      if (domNode instanceof Element && "name" in domNode) {
        ...
        if (name === "span" && attribs.type === "mentions") {
          const mention = decodeURIComponent(
            `https://matrix.to/#/${encodeURIComponent(props.id)}`
          ).match(/^https?:\/\/matrix.to\/#\/((@|#|!).+:[^?/]+)/)
          if (mention) {
              ...
              return (
                <span
                  ...
                  className={css.Mention({
                    highlight:
                      room.roomId === (mentionRoom?.roomId ?? mentionId)
                  })}
              ...
              return (
                <span
                  ...
                  className={css.Mention({
                    ...
                    otherUserMentioned: mx.getUserId() !== mentionId
                  })}
                ...
                >
                  {`${
                    getMemberDisplayName(room, mentionId) ??
                    getMxIdLocalPart(mentionId)
                  }`}
                </span>
              )
          }
        ...
        if (name === "img") {
          ...
          if (htmlSrc)
            return <img {...props} className={css.Img} src={htmlSrc} />
        }
      }
