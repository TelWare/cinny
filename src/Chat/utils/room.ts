...
export const getStateEvent = (
  ...
  room.currentState.getStateEvents(eventType, stateKey) ?? undefined

export const getStateEvents = (
  room: Room,
  eventType: StateEvent
): MatrixEvent[] => room.currentState.getStateEvents(eventType)
...
export const isUnsupportedRoom = (room: Room | null): boolean => {
  ...
  return (
    event.getContent().type !== undefined &&
    event.getContent().type !== RoomType.Space
  )
}
...
export const getSpaceChildren = (room: Room) =>
  getStateEvents(room, StateEvent.SpaceChild).reduce<string[]>(
    (filtered, mEvent) => {
...
export const getRoomToParents = (mx: MatrixClient): RoomToParents => {
  ...
  mx.getRooms()
    ...
    .forEach((room) =>
      mapParentWithChildren(map, room.roomId, getSpaceChildren(room))
    )
...
export const getNotificationType = (
  ...
  if (!roomPushRule) {
    const overrideRules = mx
      .getAccountData("m.push_rules")
      ?.getContent<IPushRules>()?.global?.override
...
export const getRoomAvatarUrl = (
  ...
  const url =
    room
      ...
      ?.getAvatarUrl(mx.baseUrl, 32, 32, "crop", undefined, false) ?? undefined
  if (url) return url
...
export const getMemberSearchStr = (
  ...
  member.rawDisplayName === member.userId
    ? mxIdToName(member.userId)
    : member.rawDisplayName,
  query.startsWith("@") || query.indexOf(":") > -1
    ? member.userId
    : mxIdToName(member.userId)
]
