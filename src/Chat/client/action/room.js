...
async function createDM(userIdOrIds, isEncrypted = true, groupName) {
  const options = {
    is_direct: true,
    creation_content: {
      type: userIdOrIds.length === 1 ? "DirectMessage" : "Groups"
    },
...
async function createRoom(opts) {
  ...
  const { usersId, name, topic, joinRule } = opts
  ...
  const options = {
    creation_content: { type: "Channels" },
    invite: Array.isArray(usersId) ? usersId : [usersId],
    name,
    topic,
    visibility,
