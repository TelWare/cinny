  ...
  _selectTabWithRoom(roomId) {
    ...
    if (roomList.isOrphan(roomId)) {
      if (roomList.directs.has(roomId)) {
        ...
        return
      } else if (roomList.favouritesDM.has(roomId)) {
        // this._selectSpace(null, true, false);
        // this._selectTab(cons.tabs.PIN, false);
        return
      } else if (roomList.favouritesChannel.has(roomId)) {
        return
      } else {
        this._selectSpace(null, true, false)
    ...
    const actions = {
      [cons.actions.navigation.SELECT_TAB]: () => {
        const roomId =
          ...
          // action.tabId !== cons.tabs.PIN &&
          action.tabId !== cons.tabs.DIRECTS
            ? action.tabId
            : null
