...
function isMedia(mE) {
  console.log('bbbbbbbbbbbbbb', mE.getContent()?.attachments?.length)
  return mE.getContent()?.attachments?.length > 0
  // return (
...
function genMediaContent(mE) {
  ...
  let mediaMXC = mE.getContent()?.attachments[0]?.url?.content_uri
  let mimeType = mE.getContent()?.attachments[0]?.info?.type
  let file = mE.getContent()?.attachments[0]?.info
  let msgType = mE.getContent()?.attachments ? mE.getContent()?.attachments[0]?.info.mtype : MsgType.File
  ...
  console.log('aaaaaaaaaaaaaaaaaa', msgType)
  switch (msgType) {
    case "m.file":
      return (
        <Media.File
    ...
    case "m.image":
      return (
        <Media.Image
          ...
          link={mx.mxcUrlToHttp(mediaMXC)}
          file={isEncryptedFile ? file : null}
          type={mimeType}
          blurhash={blurhash}
        />
      )
